...
try:
    from prompts import PROMPT_POOL            # 275 predefined prompts (prompts.py)
except Exception:
    PROMPT_POOL = [
...
ANALYSIS_TEMPLATE_PART = (
    "You are a meticulous data analyst. The document below was split into {parts} parts to fit "
    "the context window; this is part {part} of {parts}. Analyse THIS part and respond with:\n"
    "1) a concise summary of this part,\n2) key facts or numbers,\n"
    "3) notable patterns or anomalies,\n4) one actionable insight.\n\n"
    "### DOCUMENT: {name} ({kind}) — part {part}/{parts}\n{body}\n"
    "### END OF PART {part}/{parts}\n\nAnalysis:"
)
SYSTEM_PROMPT = "You are a helpful expert assistant. Answer completely and precisely."
...
def _file_parts(frec, max_parts=10):
    """Split an extracted document into prompts that each fit the context window —
    the ENTIRE document is analysed across the parts (never silently dropped)."""
    n_ctx = LOADED["n_ctx"] if LOADED else 4096
    reserve = (RUN["max_tokens"] if RUN.get("max_tokens") and RUN["max_tokens"] > 0 else 512) + 96
    budget = max(256, n_ctx - reserve - 240)          # 240 ≈ chat template + instructions
    try:
        toks = LLM.tokenize(frec["text"].encode("utf-8"), add_bos=False, special=False)
        toks = toks[: budget * max_parts]
        chunks = [LLM.detokenize(toks[i:i + budget]).decode("utf-8", "ignore")
                  for i in range(0, len(toks), budget)]
    except Exception:
        step = budget * 4
        chunks = [frec["text"][i:i + step] for i in range(0, min(len(frec["text"]), step * max_parts), step)]
    chunks = [c for c in chunks if c.strip()] or [frec["text"][: budget * 4]]
    n = len(chunks)
    out = []
    for i, c in enumerate(chunks, 1):
        tmpl = ANALYSIS_TEMPLATE if n == 1 else ANALYSIS_TEMPLATE_PART
        out.append(tmpl.format(name=frec["name"], kind=frec["kind"], body=c, part=i, parts=n))
    return out
...
def _infer_one(prompt, max_tokens, temperature, seed, params_b):
    """One prompt via the model's chat template, streamed. max_tokens <= 0/None = full
    answer until the model's natural stop (EOS) or the context window is full."""
    ...
    try:    # exact: KV tokens after generation minus generated tokens (includes chat template)
        prompt_tokens = max(int(LLM.n_tokens) - gen_tokens, 1)
    except Exception:
        prompt_tokens = _tok_count(prompt, bos=True)
...
def _build_queue(n):
    """Queue for this batch: pending uploaded-file analyses (chunked to fit ctx) first,
    then custom prompts — or the built-in pool if none are set — cycling."""
    ...
    for i in range(n):
        pos = start + i
        if pos < len(files):
            q.extend(_file_parts(files[pos]))
        else:
...
def _run_engine(n_prompts, max_tokens, temperature, seed):
    try:
        queue = _build_queue(n_prompts)
        RUN.update({"phase": "running", "target": len(queue),
        ...
        for prompt in queue:
            ...
            except Exception as ex:                       # keep other prompts running
                row = {
                    "prompt": prompt[:160], "output": f"ERROR: {type(ex).__name__}: {str(ex)[:200]}",
                    "prompt_tokens": None, "completion_tokens": None, "prompt_tps": None,
                    "gen_tps": None, "ttft_s": None, "total_time_s": None, "tops": None,
                    "peak_tops": PEAK["tops"] if PEAK else None, "top_util_pct": None,
                    "cpu_pct": None, "ram_pct": None, "ram_gb": None,
                    "gpu_pct": None, "power_w": None, "temp_c": None,
                }
            row.update({
                "idx": len(RESULTS) + 1,
...
class RunReq(BaseModel):
    n_prompts: int = 3
    max_tokens: int = -1          # -1 = full answer until the model's natural stop (EOS)
...
def api_run(req: RunReq):
    ...
    RUN["max_tokens"] = req.max_tokens if req.max_tokens <= 0 else min(8192, req.max_tokens)
...
def _row_value(r, k):
    if k == "output":
        return LAST_OUTPUTS.get(r["idx"], "")[:32000]
    v = r.get(k)
    return r["prompt"][:500] if k == "prompt" and isinstance(v, str) else v


def _avg_row():
    avg = {"prompt": "AVERAGE", "idx": "", "ts": "", "model": "", "quant": "",
           "prompt_tokens": "", "completion_tokens": "", "peak_tops": "", "output": ""}
...
def export_csv():
    ...
    for r in RESULTS:
        w.writerow(["" if _row_value(r, k) is None else _row_value(r, k) for k, _ in COLUMNS])
    if RESULTS:
        a = _avg_row()
...
def export_xlsx():
    ...
    for r in RESULTS:
        ws.append([_row_value(r, k) for k, _ in COLUMNS])
    if RESULTS:
        a = _avg_row()
    ...
    for i, (k, _) in enumerate(COLUMNS, 1):
        ws.column_dimensions[ws.cell(1, i).column_letter].width = 60 if k == "output" else (34 if k == "prompt" else 14)
    ws.freeze_panes = "A2"
    bio = io.BytesIO()
    wb.save(bio)
    bio.seek(0)
